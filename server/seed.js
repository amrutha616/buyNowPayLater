...
const EMISchedule = require("./src/models/EMISchedule");
...
const seedDatabase = async () => {
	try {
		...
		console.log("   Credit Limit: ₹10,000 | Outstanding: ₹3,666\n");
		...
		const currentCycleDueDate = new Date(
			today.getFullYear(),
			today.getMonth(),
			Math.max(1, Math.min(30, today.getDate() - 1))
		);
		...
		// Duplicate EMI test loan - intentionally mirrors Flipkart schedule
		...
		// Seed EMI schedule rows including intentional duplicates for testing
		const due1 = currentCycleDueDate;
		const due2 = currentCycleDueDate;
		...
		await Transaction.create([
			...
			},
			{
				...
				paymentMethod: "NONE",
				status: "SUCCESS",
				note: "Mode: FULL_BNPL, Installments: 3 months (Duplicate EMI test)",
			},
			{
				user: testUser._id,
				type: "REPAYMENT",
		...
		console.log("   - Flipkart Duplicate: ₹1,500 BNPL (3 months) - All pending");
		console.log("   - EMI Schedule collection seeded with intentional duplicate pending rows\n");
		console.log("🎉 Seed completed successfully!\n");
		console.log("You can now login with:");
		console.log("   - demo@phonepay.com (Fresh account, no dues)");
		console.log("   - test@phonepay.com (Has active loans + duplicate EMI fixtures for testing)\n");
